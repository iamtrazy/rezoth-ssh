...
useradd -e $exd -M -N $username && echo "$username:$password" | chpasswd && echo User $username added to the system successfully || echo Failed to add user $username please try again.
echo returned
